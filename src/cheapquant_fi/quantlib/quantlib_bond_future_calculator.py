"""QuantLib implementation of :class:`~cheapquant_fi.bond_future_calculator.BondFutureCalculator`.
...
from cheapquant_fi.bond_future_output import BondFutureBasketOutput, BondFutureOutput
from cheapquant_fi.bond_futures import MONTHS_PER_YEAR, BondFuture
from cheapquant_fi.date_utils import add_months, from_ql_date, to_ql_date
from cheapquant_fi.instruments import Bond
from cheapquant_fi.issuers import IssuerProfile
from cheapquant_fi.quantlib.quantlib_conversion_factor import conversion_factor
...
class QuantLibBondFutureCalculator:
    ...
    def compute_bond_future_analytics(
        ...
        *,
        ...
        try:
            ...
            repo_rate = self._repo_rate(request, curve_handle, settlement, delivery)
    ...
    def _repo_rate(
        ...
        Falls back to the discount curve's own simple forward rate when no
        repo term structure was supplied.
        """
        convention = request.bond_future.convention
        if request.repo_term_structure is not None:
            return request.repo_term_structure.rate_for(
                delivery,
