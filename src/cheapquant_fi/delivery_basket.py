...
from cheapquant_fi.date_utils import whole_months_between
...
class BasketMember:
    """One deliverable bond, with an optional hard-coded conversion factor.
...
class DeliveryBasket:
    ...
    # Membership
    # ------------------------------------------------------------------ #
    def add(self, bond: Bond, conversion_factor: float | None = None) -> None:
        ...
        self.members = (*self.members, BasketMember(bond, conversion_factor))
...
class DeliveryBasketManager:
    ...
    Mirrors :class:`~cheapquant_fi.bond_manager.BondManager`: a singleton keyed
...
def parse_fut_command(text: str) -> FutParseResult | None:
    ...
    if len(tokens) > 2:
        return FutParseResult(
            kind="invalid",
            message="/fut takes a basket name or contract code and an optional trade date.",
        )

    target = tokens[0]
    trade_date: date | None = None
    if len(tokens) == 2:
        if not _ISO_DATE_RE.match(tokens[1]):
            return FutParseResult(
                kind="invalid",
                message=f"Cannot read {tokens[1]!r} as a trade date; expected YYYY-MM-DD.",
            )
        try:
            trade_date = date.fromisoformat(tokens[1])
        except ValueError as exc:
            return FutParseResult(kind="invalid", message=str(exc))

    return FutParseResult(kind="analytics", target=target, trade_date=trade_date)
