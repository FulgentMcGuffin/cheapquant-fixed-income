...
from cqfi.cache.registry import get_cache_registry
from cqfi.config import get_runtime_settings
...
def cache_bond_future_analytics(fn: F) -> F:
    """Persist ``compute_bond_future_analytics`` when ``use_quant_cache`` is true."""
    ...
    def wrapper(
        ...
        **kwargs: Any,
        ...
        result: BondFutureBasketOutput = fn(
            self, request, market, *args, curve_label=curve_label, **kwargs
        )
        if not get_runtime_settings().use_quant_cache:
            return result
